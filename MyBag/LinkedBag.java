...
// or null otherwise.
    ...
    public boolean equals(LinkedBag<T> aBag) {
        ...
        if(firstNode == null && aBag.firstNode == null) {
        	return true;
        }
        if((firstNode == null && aBag.firstNode != null) || (firstNode != null && aBag.firstNode == null) || (numberOfEntries != aBag.numberOfEntries)) {
        	return false;
        }
        
        Node nextNodeF = firstNode;
       for(int i = 0; i < numberOfEntries; i++ ) {
    	   T checkData = nextNodeF.data;
    	   int checkDataFrequency = getFrequencyOf(checkData);
    	   if(aBag.contains(checkData)) {
    		   if(aBag.getFrequencyOf(checkData) == checkDataFrequency) {
    			   result = true;
    		   }else {
    			   result = false;
        		   break;
    ...
    public void removeDuplicates() {
    	...
    	for ( int i = 0 ; i < numberOfEntries ; i++) {
    		if(!newBag.contains(newNode.data)) {
    			newBag.add(newNode.data);
    		}
    		if(newNode.next != null) {
        		newNode = newNode.next;
        	}  
    	}
    	firstNode = newBag.firstNode;
    	numberOfEntries = newBag.numberOfEntries;
        return;
